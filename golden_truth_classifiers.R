#Crowdsourcing project - part 1 - Golden truth classifiers
...
paritionedData <- partitionData(data, fractionOfDataForTraining)
trainData   <- paritionedData$trainData
testData    <- paritionedData$testData


#Attribute selection
#Use the gain ratio to find which attribute(s) have the highest mutual information 
#to be selected for the model creation process

library( RWeka )
gainRatioResult <- GainRatioAttributeEval( class ~ . , data = trainData )
