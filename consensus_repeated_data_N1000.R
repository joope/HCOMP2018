...
#Empty matrix of 1000 workers. 
workers_l<-matrix(NA,nrow=N, ncol=1000)

#First 100 workers are true experts, noise = 5%
for (i in 1:100) {
...
#Second part of 100 workers are experts, noise = 20%
for (i in 101:200) {
  nois = rbinom(N,1,0.2)
  lbs = mushrooms_Dexp[1:N,]$label
  lbs[mushrooms_Dexp[1:N,]$label==1 & nois]=2
...
#Third part of 350 workers are amateurs, noise = 40%
for (i in 201:550) {
  nois = rbinom(N,1,0.4)
  lbs = mushrooms_Dexp[1:N,]$label
  lbs[mushrooms_Dexp[1:N,]$label==1 & nois]=2
